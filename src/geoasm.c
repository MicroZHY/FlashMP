...
#include <../src/ksp/pc/impls/asm/asm.h> /*I "petscpc.h" I*/
  ...
  #define PetscCheck(condition, comm, errcode, msg, ...) \
    do { \
      if (!(condition)) { \
        PetscError(comm, __LINE__, PETSC_FUNCTION_NAME, __FILE__, errcode, PETSC_ERROR_INITIAL, msg, ##__VA_ARGS__); \
        return errcode; \
      } \
    } while (0)
#else
  ...
  #define PetscCheck(condition, comm, errcode, msg, ...) \
    do { \
      if (!(condition)) { \
        SETERRQ(comm, errcode, msg, ##__VA_ARGS__); \
      } \
    } while (0)
#endif
...
static PetscErrorCode PCSetUp_GEOASM(PC pc)
{
  PC_ASM     *osm = (PC_ASM *)pc->data;
  PetscBool   flg;
  PetscInt    i, m, m_local;
  ...
  if (!pc->setupcalled) {
    ...
    if (osm->overlap > 0) {
	  ...
	  ////////////////////////////////
	  if(pc->dm){
		  osm->sort_indices = PETSC_FALSE ;
		  for (i =0; i< osm->n_local_true; i++) osm->is[i] =NULL;
		  osm->n_local_true = 1 ;
		  //
		  PetscInt sw,dimx,dimy,dimz;
		  PetscCall(DMDAGetInfo(pc->dm, NULL, &dimx, &dimy, &dimz, NULL,NULL,NULL,NULL,&sw, NULL,NULL,NULL,NULL));
		  if(osm->overlap != sw){
			  PetscCall(PetscPrintf(PETSC_COMM_WORLD, "change ASM overlapsize to %d according to DM stencil width !\n", sw));
			  osm->overlap = sw;
		  }
		  Vec global_x,local_x;
		  PetscCall(DMCreateGlobalVector(pc->dm, &global_x));
		  PetscScalar  ***var_x0;
		  PetscCall(DMDAVecGetArray(pc->dm, global_x, &var_x0));
		  PetscInt xs, ys, zs, xm, ym, zm;
		  PetscCall(DMDAGetCorners(pc->dm, &xs, &ys, &zs, &xm, &ym, &zm));  
          PetscInt k,j,i, irank, ishift;
		  PetscCallMPI(MPI_Comm_rank(PETSC_COMM_WORLD, &irank));
		  ishift = irank * zm*ym*xm*3 ;
	      // PetscCall(PetscSynchronizedPrintf(PETSC_COMM_WORLD, "irank = %d  %d  %d  %d   %d  \n", irank, xm, ym,zm, ishift)); 
	      // PetscCall(PetscSynchronizedFlush(PETSC_COMM_WORLD, PETSC_STDOUT));		  
		  for (k = zs; k < zs + zm; k++) {
			  for (j = ys; j < ys + ym; j++) {
				  for (i = xs; i < xs + xm; i++) {
					  var_x0[k][j][3*i+0] = ishift + 0+ (i-xs)*3+(j-ys)*xm*3+(k-zs)*ym*xm*3;//i*3+j*dimx*3+k*dimy*dimx*3+0;
					  var_x0[k][j][3*i+1] = ishift + 1+ (i-xs)*3+(j-ys)*xm*3+(k-zs)*ym*xm*3;//i*3+j*dimx*3+k*dimy*dimx*3+1;
					  var_x0[k][j][3*i+2] = ishift + 2+ (i-xs)*3+(j-ys)*xm*3+(k-zs)*ym*xm*3;//i*3+j*dimx*3+k*dimy*dimx*3+2;			  
				  }
			  }
		  }
		  PetscCall(DMDAVecRestoreArray(pc->dm, global_x, &var_x0)); 		      		  
		  PetscCall(DMGetLocalVector(pc->dm, &local_x));
		  PetscCall(DMGlobalToLocalBegin(pc->dm, global_x, INSERT_VALUES, local_x));
          PetscCall(DMGlobalToLocalEnd(pc->dm, global_x, INSERT_VALUES, local_x)); 
		  PetscCall(DMDAVecGetArray(pc->dm, local_x, &var_x0));
		  PetscInt *indices, count, icount=0; //      *count, nidx, *indices, *newidx
		  count = 3*(xm+2*sw)*(ym+2*sw)*(zm+2*sw);
	      PetscCall(PetscMalloc1(count, &indices));
		  for (k = zs -sw; k < zs + zm +sw; k++) {
			  for (j = ys -sw; j < ys + ym +sw; j++) {
				  for (i = xs -sw; i < xs + xm +sw; i++) {
					  indices[icount++] = (PetscInt)var_x0[k][j][3*i+0] ;
					  indices[icount++] = (PetscInt)var_x0[k][j][3*i+1] ;
					  indices[icount++] = (PetscInt)var_x0[k][j][3*i+2] ;	  
				  }
			  }
		  }		  
		  PetscCall(DMDAVecRestoreArray(pc->dm, local_x, &var_x0));	
		  PetscCall(DMRestoreLocalVector(pc->dm, &local_x));	  
		  PetscCall(ISCreateGeneral(PETSC_COMM_SELF, count, indices, PETSC_COPY_VALUES, &osm->is[0]));
		  PetscCall(PetscFree(indices));
   		  // PetscInt rank;
		  // PetscCallMPI(MPI_Comm_rank(PETSC_COMM_WORLD, &rank));
		  // if(rank==0) PetscCall(ISView(osm->is[0],  PETSC_VIEWER_STDOUT_SELF));   
	  }else{
	...
	//PetscCall(PetscSynchronizedPrintf(PETSC_COMM_WORLD, "is local size = %d   \n", m)); 
	//PetscCall(PetscSynchronizedFlush(PETSC_COMM_WORLD, PETSC_STDOUT));
  ...
  /*
     Extract out the submatrices
  */
  	// PetscCall(PetscSynchronizedPrintf(PETSC_COMM_WORLD, "irun1  \n")); 
	// PetscCall(PetscSynchronizedFlush(PETSC_COMM_WORLD, PETSC_STDOUT));
  PetscCall(MatCreateSubMatrices(pc->pmat, osm->n_local_true, osm->is, osm->is, scall, &osm->pmat));
  	// PetscCall(PetscSynchronizedPrintf(PETSC_COMM_WORLD, "run2  \n")); 
	// PetscCall(PetscSynchronizedFlush(PETSC_COMM_WORLD, PETSC_STDOUT));
  if (scall == MAT_INITIAL_MATRIX) {
...
/*   {
	  	PetscInt rank;
		PetscCallMPI(MPI_Comm_rank(PETSC_COMM_WORLD, &rank));
		if(rank==0) PetscCall(MatView(osm->pmat[0],  PETSC_VIEWER_STDOUT_SELF));
  } */
  ...
  if (!pc->setupcalled) {
    ...
    // 替换 MatGetVecType
const char *mattype;
PetscCall(MatGetType(osm->pmat[0], &mattype));
 
if (!strcmp(mattype, MATMPIAIJ) || !strcmp(mattype, MATMPIBAIJ)) {
    vtype = VECMPI; // 并行矩阵使用 VECMPI
} else if (!strcmp(mattype, MATSEQAIJ) || !strcmp(mattype, MATSEQBAIJ)) {
    vtype = VECSEQ; // 串行矩阵使用 VECSERIAL
} else {
    vtype = VECSTANDARD; // 其他情况使用默认类型
}
